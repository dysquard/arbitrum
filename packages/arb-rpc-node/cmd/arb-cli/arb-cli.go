...
func createChain(rollupCreator, owner, sequencer ethcommon.Address, blockTime float64) error {
	...
	receipt, err := ethbridge.WaitForReceiptWithResults(context.Background(), config.client, config.auth.From, tx, "CreateRollup")
	if err != nil {
		return err
	}
	fmt.Println("Transaction completed successfully")
...
func enableValidators(rollup ethcommon.Address, validators []ethcommon.Address) error {
	...
	fmt.Println("Waiting for receipt for", tx.Hash())
	_, err = ethbridge.WaitForReceiptWithResults(context.Background(), config.client, config.auth.From, tx, "CreateRollup")
	if err != nil {
		return err
	}
	fmt.Println("Transaction completed successfully")
...
func enableL1Addresses(rollup, whitelist ethcommon.Address, users []ethcommon.Address) error {
	...
	fmt.Println("Waiting for receipt for", tx.Hash())
	_, err = ethbridge.WaitForReceiptWithResults(context.Background(), config.client, config.auth.From, tx, "SetWhitelistEntries")
	if err != nil {
		return err
	}
	fmt.Println("Transaction completed successfully")
...
func deposit(inboxAddress ethcommon.Address, value *big.Int, submissionPrice *big.Int) error {
	...
	fmt.Println("Waiting for receipt for", tx.Hash())
	_, err = ethbridge.WaitForReceiptWithResults(context.Background(), config.client, config.auth.From, tx, "DepositEth")
	if err != nil {
		return err
	}
	fmt.Println("Transaction completed successfully")
...
func deployContract(rawTx string) error {
	...
	fmt.Println("Waiting for receipt")
	_, err = ethbridge.WaitForReceiptWithResults(context.Background(), config.client, config.auth.From, tx, "DeployContract")
	if err != nil {
		return err
	}
	fmt.Println("Transaction completed successfully")
